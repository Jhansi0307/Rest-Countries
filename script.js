...
.then((data)=>{
    // console.log(data);
    //here not able to get flags in console because flags are figures how its visible in console.
    for(let i=0;i<data.length;i++){
        ...
        div_2.innerHTML=`Name: ${data[i].name.common}`
        ...
        div_3.innerHTML=`Subregion: ${data[i].subregion}`

        let d4=document.createElement('div')
        d4.classList.add('card-text')
        div_body.appendChild(d4)
        let div_4 = document.createElement('div')
        div_4.innerHTML=`Population: ${data[i].population}`
